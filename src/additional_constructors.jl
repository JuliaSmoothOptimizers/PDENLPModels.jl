function GridapPDENLPModel(
    ...
    if NRJ <: NoFETerm && typeof(lvar) <: AbstractVector && typeof(uvar) <: AbstractVector
        ...
        lv, uv = bounds_functions_to_vectors(
            Y,
            VoidFESpace(),
            Ypde,
            tnrj.trian,
...
function GridapPDENLPModel(
    ...
    tnrj =
        nparam > 0 ? MixedEnergyFETerm(f, trian, quad, nparam) :
        EnergyFETerm(f, trian, quad)
...
function GridapPDENLPModel(
    ...
    lvark::AbstractVector = -T(Inf) * ones(
        T,
        max(length(x0) - num_free_dofs(Ypde) - num_free_dofs(Ycon), 0),
    ),
    uvark::AbstractVector = T(Inf) * ones(
        T,
        max(length(x0) - num_free_dofs(Ypde) - num_free_dofs(Ycon), 0),
    ),
    ...
    elseif NRJ != NoFETerm
        fun_lvar, fun_uvar = bounds_functions_to_vectors(
            Y,
            Ycon,
            Ypde,
            tnrj.trian,
