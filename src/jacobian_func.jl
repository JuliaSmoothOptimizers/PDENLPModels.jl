...
"""
function fill_jac_coo_symbolic!(I,J,a::Gridap.FESpaces.GenericSparseMatrixAssembler,cellidmatdata;n=0)
  ...
  for (cellidsrows, cellidscols) in zip(term_to_cellidsrows, term_to_cellidscols)
    ...
    if length(cell_cols) > 0
      ...
      nini = Gridap.FESpaces._allocate_matrix!(a.matrix_type, nini, I, J, 
...
function allocate_coo_jac!(op   :: Gridap.FESpaces.FEOperatorFromTerms,
                           Y    :: FESpace,
                           Xpde :: FESpace,
                           Ypde :: FESpace,
                           Ycon :: FESpace,
...
The motivation is to avoid the use of the yet unknown values.
"""
function count_nnz_coo_short(a :: Gridap.FESpaces.GenericSparseMatrixAssembler,
                             cellidmatdata)
  ...
  for (cellidsrows,cellidscols) in zip(cellidmatdata...)
    ...
    if length(cell_cols) > 0
      ...
      n += Gridap.FESpaces._count_matrix_entries(a.matrix_type, rows_cache, cols_cache,
...
function count_nnz_jac(op   :: Gridap.FESpaces.FEOperatorFromTerms,
                       Y    :: FESpace,
                       Xpde :: FESpace,
                       Ypde :: FESpace,
...
function count_nnz_jac(op   :: AffineFEOperator,
                       Y    :: FESpace,
                       Xpde :: FESpace,
                       Ypde :: FESpace,
                       Ycon :: FESpace)
...
function _from_terms_to_jacobian_vals!(op   :: AffineFEOperator,
                                       x    :: AbstractVector{T},
                                       Y    :: FESpace,
                                       Xpde :: FESpace,
                                       Ypde :: FESpace,
...
function _from_terms_to_jacobian_vals!(op   :: Gridap.FESpaces.FEOperatorFromTerms,
 ...
 for term in op.terms
   ...
   Main.PDENLPModels._jac_from_term_to_terms!(term, κ, yu, yh, uh,
                                  dyu, dy, du, v,
                                  w,  r,  c,
                                  wu, ru, cu,
                                  wy, ry, cy)
...
function assemble_jac_vals!(mat,a::Gridap.FESpaces.GenericSparseMatrixAssembler,matdata; n = 0)
  ...
  for (cellmat_rc,cellidsrows,cellidscols) in zip(matdata...)
    ...
    nini = _assemble_jac!(mat, vals_cache, rows_cache, cols_cache, cell_vals, cell_rows, cell_cols, a.strategy, n = nini)
  end
  nini
end

@noinline function _assemble_jac!(mat, vals_cache, rows_cache, cols_cache, cell_vals, cell_rows, cell_cols, strategy; n = 0)
  nini = n
...
function _jac_from_term_to_terms_id!(term  :: Gridap.FESpaces.FETerm,
                                     r     :: AbstractVector,
                                     c     :: AbstractVector,
                                     ru    :: AbstractVector,
                                     cu    :: AbstractVector,
...
function _jac_from_term_to_terms_id!(term  :: Union{Gridap.FESpaces.LinearFETerm, Gridap.FESpaces.AffineFETermFromIntegration},
                                     r     :: AbstractVector,
                                     c     :: AbstractVector,
                                     ru    :: AbstractVector,
                                     cu    :: AbstractVector,
...
function _jac_from_term_to_terms_id!(term  :: Union{Gridap.FESpaces.NonlinearFETermWithAutodiff,Gridap.FESpaces.NonlinearFETerm},
                                     r     :: AbstractVector,
                                     c     :: AbstractVector,
                                     ru    :: AbstractVector,
                                     cu    :: AbstractVector,
...
function _jac_from_term_to_terms_id!(term  :: Gridap.FESpaces.FESource,
                                     r     :: AbstractVector,
                                     c     :: AbstractVector,
                                     ru    :: AbstractVector,
                                     cu    :: AbstractVector,
...
function _jac_from_term_to_terms_u_id!(term :: Union{Gridap.FESpaces.NonlinearFETermWithAutodiff,Gridap.FESpaces.NonlinearFETerm},
                                       r    :: AbstractVector,
                                       c    :: AbstractVector)
...
function _jac_from_term_to_terms_y_id!(term :: Union{Gridap.FESpaces.NonlinearFETermWithAutodiff,Gridap.FESpaces.NonlinearFETerm},
                                       r    :: AbstractVector,
                                       c    :: AbstractVector)
