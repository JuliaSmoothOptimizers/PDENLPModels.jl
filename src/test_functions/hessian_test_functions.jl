...
function hessian_test_functions(nlp::GridapPDENLPModel; udc = false, tol = 1e-15)
    ...
    if (typeof(nlp.tnrj) <: MixedEnergyFETerm && nlp.tnrj.inde) ||
       typeof(nlp.tnrj) <: NoFETerm
        ...
        nnz_hess_k =
            Int(nlp.nparam * (nlp.nparam + 1) / 2) +
            (nlp.meta.nvar - nlp.nparam) * nlp.nparam
    end
    ...
    @test_throws DimensionError hess(nlp, vcat(nlp.meta.x0, 1.0))
    ...
    _Hxv = hprod(nlp, nlp.meta.x0, ones(nlp.meta.nvar), obj_weight = 0.5)
    _Hxvr = hprod(nlp, xr, ones(nlp.meta.nvar), obj_weight = 1.0)
    ...
    hop1 = hess_op(nlp, nlp.meta.x0, obj_weight = 0.5)
    hopr = hess_op(nlp, xr, obj_weight = 1.0)
