...
function bounded_2d_variational_problem(n, domain, x0, xf, xmin, xmax, f)
    ...
    V1 = TestFESpace(reffe=:Lagrangian, order=1, valuetype=Float64,
                           conformity=:H1, model=model, dirichlet_tags=["diri0","diri1"])
    ...
    xin = zeros(nU0 + nU1)
    nlp = GridapPDENLPModel(xin, f, trian, quad, U, V, lvar = xmin * ones(nU0+nU1), uvar = xmax*ones(nU0+nU1))
    return nlp
end
...
function bounded_2D_optimal_control(domain, n, f, con1, con2, umin, umax, xmin, xmax, x0, xf)
